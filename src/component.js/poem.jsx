...
export default function Poem(){
  ...
  const renderPoem = () => {
    if(!isSubmitted){
        return <em className="flex justify-center items-center p-3"> Fill in the form to reveal your Poem...</em>
    }
    return(
        <>
  ...
  return (
    ...
    <div className="flex justify-center items-center bg-gray-700  p-8 w-96  mx-auto rounded-xl">
        <form action="Submit" onSubmit={handleSubmit}className="flex flex-col  gap-5 ">
        <input type="text" value={firstName} className="p-2 border rounded-xl w-80" placeholder="First Name" onChange = {(e) => setFirstName(e.target.value)}/>
        <input type="text" value={lastName} className="p-2 border rounded-xl w-80"  placeholder="Last Name" onChange ={(e) => setLastName(e.target.value)}/>
        <input  className="p-2 border rounded-xl w-80" type="text" value={adress} placeholder="Adress"  onChange = { (e) => setAdress(e.target.value)}/>
